...
public class TrackTokenUsageCommandHandler : ICommandHandler<TrackTokenUsageCommand, bool>
{
    ...
    public async Task<bool> Handle(TrackTokenUsageCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var entity = MapModelToEntity(request.Record);
    ...
    private static Infrastructure.Persistence.Entities.TokenUsageRecord MapModelToEntity(
        Models.TokenUsage.TokenUsageRecord model)
    {
        return new Infrastructure.Persistence.Entities.TokenUsageRecord
        {
            Id = model.Id,
